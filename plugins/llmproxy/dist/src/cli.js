...
export async function main(argv, env = process.env) {
    ...
    try {
        if (cmd === "init")
            return runInit(env);
        if (cmd === "test")
            return await runTest(env);
        if (cmd === "delegate")
            return runDelegate(rest, env);
        if (cmd === "config")
            return runConfig(rest[0] ?? "", rest.slice(1), env);
        if (STACK.has(cmd))
            return await runStack(cmd, rest, env);
        // otherwise: treat cmd as a brain name to launch (chat/deep/coder/...)
        const cfg = loadBrains(resolvePaths(env).brainsYaml);
        if (cfg.brains[cmd])
            return runLaunch(cmd, rest, env);
