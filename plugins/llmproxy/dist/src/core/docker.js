...
export function composeArgs(paths) {
...
export function ensureDocker() {
    ...
    if (r.error)
        throw new Error("docker not found on PATH — install Docker and retry.");
    if (r.status !== 0)
        throw new Error("docker daemon not reachable — start Docker and retry.");
}
export function runCompose(paths, args) {
    const r = spawnSync("docker", [...composeArgs(paths), ...args], { stdio: "inherit" });
    return r.status ?? 1;
}
export function liveliness(port, timeoutMs = 3000) {
    return new Promise((resolve) => {
        const req = http.get({ host: "127.0.0.1", port, path: "/health/liveliness", timeout: timeoutMs }, (res) => {
            res.resume();
